function client_connected_to_server(socket)
	print("client connected to server")
	print("----\n")
end

function client_reconnected_to_server(socket, error_code)
	print("client reconnected to server")
	print(error_code)
	print("----\n")
end

function client_disconnected_from_server(socket, error_code)
	print("client disconnected to server")
	print(error_code)
	print("----\n")
end

function received_from_client(socket, request)
	--[[
		fnd = { string.find(request, "data=") }
		if #fnd ~= 0 then
			control = string.sub(request, fnd[2] + 1)
			local key = "thisIsWh3Ecrypti"
			print("control: ", control)
			local asc = encoder.fromHex(control)
			local msg = crypto.decrypt("AES-ECB", "thisIsWh3Ecrypti", asc)
			print("The sent message is: ", msg)
			socket:send("HTTP/1.1 200 OK\r\n")
			socket:send(msg)
		end
	--]]
    print("request:\n", request)
	--local start_end = { string.find(request, "data=") }
	--local data = string.sub(request, start_end[2] + 1)
	--local encrypted_string = encoder.fromHex(data)
...
function sent_to_client(socket)
	print("sent to client")
	socket:close()
	print("now the socket is closed")
end

function respond_to_successful_connection(socket)
